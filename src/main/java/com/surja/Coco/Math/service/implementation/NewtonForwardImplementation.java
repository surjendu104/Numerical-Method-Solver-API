...
@Service
public class NewtonForwardImplementation implements EqualIntervalInterpolationService {
    @Override
    public double interpolationResult(double[][] table, double u) {
        ...
        while (i < table[0].length) {
            ans += calculateU(u, i) * table[i][i] / factorial(i);
            i++;
        }
        return ans;
    }
    ...
    public double findU(double[] points, double valuePoint) {
        return (valuePoint-points[0])/(points[1]-points[0]);
    }
