...
public class SolutionOfODEImplementation implements SolutionOfODEService {
    @Override
    public double[][] eulersMethod(String equation, double valueOfX, double valueOfY, double height, int partitions, double valuePoint) {
    ...
    @Override
    public double[][] eulersModifiedMethod(String equation, double valueOfX, double valueOfY, double height, int partitions, double valuePoint) {
        boolean isCallSuccessful = License.iConfirmNonCommercialUse("Person 1");
        Function f = new Function("f(x, y) = "+equation);
        double[][] result = new double[partitions+1][2];
        result[0][0] = valueOfX;
